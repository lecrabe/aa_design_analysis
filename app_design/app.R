    ...
    tabItem(tabName = 'Responsedesign',
        ...
        fluidRow(
          ...
          box(
            ...
            downloadButton('download_CE_shp', 
                           label='Download sampled polygons')
...
server <- function(input, output,session) {
  ...
  mapareaInput1 <-  reactive({
    ...
    if (input$IsManualAreaRaster!=T){
      ...
      if(input$rasterarea == "r"){
        ...
        beginCluster()
        withProgress(
          message= 'Computing frequency values..... this may take a while especially if the raster is big',
          value = 0,
          {
            setProgress(value=.1)
  ...
  mapareaInput2 <- reactive({
    ...
    #value_attr <- input$value_attribute2
    print(class_attr)
    ...
    # write.csv(maparea, paste0(input$dirname,"/area_shp.csv"), row.names=F)
    
    ############### Output the result as a data.frame
    mapareaInput2 <- maparea[, c(2,1,4)]
  ...
  output$selectUI_cat_hi <- renderUI({
    ...
    # lo_ua <- input$cat_lo
    # categories <- as.list(unique(maparea$map_class[!maparea$map_class %in% lo_ua]))
  ...
  output$sampling_table <- renderTable({
    ...
    if(input$IsManualSampling == T){
      ...
      # df<-read.csv(paste(input$dirname,"/",input$manualSampling$name,sep=""),header = T)
      }else{
        df<- strat_sample()
        df<-df[,c(1,2,3,8,9,12,13)]
      }
    df <- df[,c(1,5,6,7)]
    names(df)<- c('Map Value','Proportional','Adjusted','Final')
    df
    },
  ...
  all_points <- reactive({
    if(mapType()== "raster_type"){
      ...
      if(input$IsManualSampling == T){
        ...
        # rp <- read.csv(paste0(input$dirname,"/", input$manualSampling$name), header = T)
      }
      map <- lcmap()
 ...
 #   if(mapType()== "raster_type"){
  ...
  CEfile <- reactive({
    ...
    ################ Add the map code
    m$class <- map_code
    write.csv(m,paste0(outdir(),"/response.csv"),row.names=F)
  ...
  output$download_CE_shp <- downloadHandler(
    ...
    content = function(file) {
      ...
      # writeOGR(to_export, dsn=paste0(input$dirname, input$basename_CE, ".shp"), layer=input$basename_CE, 
      #          driver="ESRI Shapefile")
      zip(zipfile=paste0(outdir(),"/", input$basename_CE, ".zip"), 
          files=Sys.glob(paste0(outdir(),"/", input$basename_CE, ".*")))
      # zip(zipfile=paste0(input$dirname, input$basename_CE, ".zip"), 
      #     files=Sys.glob(paste0(input$dirname, input$basename_CE, ".*")))
      file.copy(paste0(outdir(),"/",input$basename_CE,".zip"), file)
      
      # file.copy(paste0(input$dirname, input$basename_CE,".zip"), file)
      #       if (length(Sys.glob("shpExport.*"))>0){
      #         file.remove(Sys.glob("shpExport.*"))
      #       }
    }
